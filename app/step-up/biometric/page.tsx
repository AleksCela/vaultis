    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12" style={{ zIndex: 1, position: 'relative' }}>
      <div className="w-full max-w-sm space-y-5">
        ...
        <div className="card p-4 space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--blue)' }}>Why Biometric Is Required</p>
          <div className="space-y-2 text-xs" style={{ color: 'var(--text-muted)' }}>
            ...
            <div className="flex items-start gap-2">
              ...
              <p>Under <strong style={{ color: 'var(--text)' }}>EU PSD2 Directive (SCA)</strong>, high-value or anomalous transfers require Strong Customer Authentication — something you <em>are</em> (biometric) or something you <em>know</em> (PIN).</p>
            </div>
            <div className="flex items-start gap-2">
              <span style={{ color: 'var(--green)' }} className="shrink-0">🛡</span>
              <p>Biometric data never leaves your device. We verify via <strong style={{ color: 'var(--text)' }}>WebAuthn (W3C standard)</strong> — no raw fingerprint or face data is stored or transmitted.</p>
            </div>
          </div>
        </div>

        {error && (
          <div className="text-sm px-3 py-2.5 rounded-lg" style={{ background: 'rgba(239,68,68,0.1)', color: 'var(--red)', border: '1px solid rgba(239,68,68,0.2)' }}>
            {error}
          </div>
        )}
